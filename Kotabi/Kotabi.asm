...
; Strings To Be Displayed for the user
	...
	exitMsgLen dw 27
	exitMsg2 db "      Press any key ",10,13
	exitMsg2Len dw 27
...
; ========== Game Theme ===================
; Kotabi Face :)
	KotabiFace db 1h ; Kotabi-san 
	KotabiColor db 0Eh ; Yellow
...
proc buildInitialScreen
	...
	add BX,0101h
	CALL setXY
	MOV DX, offset DoneByMsg2
	MOV CX, DoneByMsg2Len
	CALL printLine
	CALL setXY
...
@@putBlocks:
    ...
    MOV AL, 00H      ; Lines To Scroll
    MOV BH, borderColor   ; Use the updated yellow color

    INT 10H  ; draw block from beginning to end
    ADD SI, 2
...
Proc buildBoard
    ...
    MOV AL, 00H            ; Clear entire region
    MOV BH, borderColor    ; Set border color
    MOV CX, boardBeginingXY  ; Top-left corner
    MOV DX, boardEndingXY    ; Bottom-right corner
    INT 10H                ; Call BIOS to perform the operation
	MOV AH,06H
	MOV AL,00H
	MOV BH,graseColor
	add cx, 0101h
